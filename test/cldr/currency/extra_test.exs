defmodule Cldr.Currency.ExtraTest do
  ...
  describe "currency_code/1" do
    test "returns currency codes as expected" do
      assert CurrencyExtra.currency_codes(:legal_tender) |> Enum.count() == 158
      assert CurrencyExtra.currency_codes(:legal_tender_extra) |> Enum.count() == 3
      assert CurrencyExtra.currency_codes(:legal_tender_deprecated) |> Enum.count() == 124
      ...
      assert CurrencyExtra.currency_codes(:international) |> Enum.count() == 166
    end
